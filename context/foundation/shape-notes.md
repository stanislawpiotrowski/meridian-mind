...
- The student returns for a second session 1+ days later and the SRS engine has prioritized the items they got wrong in the previous session so those items appear earlier and more often in the new queue.
...
- **FR-016**: When a student starts a new session against a previously-studied set, the queue is ordered by an SRS engine that prioritizes items the student got wrong or has not seen recently. Priority: must-have
