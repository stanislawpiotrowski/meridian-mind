...
- [x] 1.1 Type checking passes: `npm run typecheck` — a6c0af0
- [x] 1.2 Linting passes (changed files): `npx eslint src/lib/prioritize.ts src/lib/dashboard.ts` — a6c0af0

#### Manual

- [x] 1.3 `isDue` correct for never-seen, stale/missed, and recent-accurate items — a6c0af0
- [x] 1.4 `computeStreak` correct for consecutive, gap, stale-only, and empty inputs — a6c0af0
- [x] 1.5 `getDashboardData` shape matches `DashboardData`; per-set `dueCount` reflects seeded history — a6c0af0
