...
**F3 addendum (impl-review)**: The original `study_history` policy was `for all to authenticated`, which conflicts with the "append-only attempts log" contract by allowing owner UPDATE/DELETE via PostgREST. Follow-up migration `20260531003200_study_history_append_only.sql` drops `study_history_owner` and replaces it with `study_history_insert_owner` (for insert) + `study_history_select_owner` (for select), so the only way a row disappears is cascade from `sets` / `study_sessions`.
...
#### 5. ESLint ignore for the generated types file (addendum, recorded in impl-review F2)

**File**: `eslint.config.js`

**Intent**: Exclude `src/db/database.types.ts` from lint. The file is regenerated by `supabase gen types typescript --linked` and is not human-edited, so linting it adds ~295 lines of noise to every `npm run lint` run.

**Contract**: One additional config object `{ ignores: ["src/db/database.types.ts"] }` inserted after the gitignore-derived ignores, with a comment explaining the file is generated. No other eslint changes.
...
- [x] 2.3 Lint passes: `npm run lint` — adapted: no new errors introduced by F-01; pre-existing CRLF baseline (1000 errors on un-touched src files from prior commits) is out of scope. The pre-commit `lint-staged` hook auto-fixes staged files at commit time. — 832277c (corrected count after `.claude/worktrees/` was gitignored in impl-review F1)
